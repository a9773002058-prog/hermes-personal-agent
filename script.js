...
if (riskWidget) {
  ...
  form?.addEventListener("submit", (event) => {
    ...
    categoryNode.textContent = `Ваш риск: ${meta.label}`;
    messageNode.textContent =
      ...
      `SCORE показывает вероятность того, что в ближайшие 10 лет у человека может случиться инфаркт, инсульт или смерть от сердечно-сосудистого заболевания. Чем выше процент, тем выше необходимость активной профилактики и снижения LDL (ЛПНП, «плохого» холестерина).\n\n` +
      `Что делать:\n- ${meta.actions.join("\n- ")}\n\n` +
      `Целевой LDL (ЛПНП, «плохой» холестерин):\n${meta.ldlTarget}\n\n` +
      `Рекомендация:\n${meta.recommendation}` +
      `${ldlKnown ? "" : "\n\nLDL (ЛПНП, «плохой» холестерин) не указан, поэтому целевой уровень показан по категории риска."}` +
      `${unknownNote}\n\nHDL (ЛПВП) — это «хороший» холестерин. LDL (ЛПНП) — это «плохой» холестерин, ключевой показатель для профилактики атеросклероза.\n\nЭто не диагноз и не индивидуальное назначение лечения. Точная картина требует консультации врача, осмотра и анализов.`;
